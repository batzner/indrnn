...
def build_rnn(inputs, phase):
  ...
  layer_input = tf.transpose(inputs, [1, 0, 2])
  layer_output = None
  ...
  for layer in range(1, NUM_LAYERS + 1):
    ...
    input_kernel = tf.get_variable("input_kernel%d" % layer,
                                   [input_depth, NUM_UNITS],
                                   initializer=input_init)
    recurrent_kernel = tf.get_variable("recurrent_kernel%d" % layer,
                                       [NUM_UNITS],
                                       initializer=recurrent_init)
    recurrent_kernel = tf.clip_by_value(recurrent_kernel,
                                        -RECURRENT_MAX,
                                        RECURRENT_MAX)
    bias = tf.get_variable("bias%d" % layer, [NUM_UNITS],
                           initializer=tf.zeros_initializer)

    # Build the layer
    # layer_input is [T, B, I]
    layer_input = tf.reshape(layer_input,
                             [TIME_STEPS * batch_size, input_depth])
    activation = tf.matmul(layer_input, input_kernel)
    activation = tf.reshape(activation, [TIME_STEPS, batch_size, NUM_UNITS])

    init_state = tf.zeros([batch_size, NUM_UNITS])

    def time_step(last_state, current_input):
      act = current_input
      act += tf.multiply(last_state, recurrent_kernel)
      act += bias
      return tf.nn.relu(act)

    layer_output = tf.scan(time_step, activation, initializer=init_state)
  ...
  # layer_output has shape [TIME_STEPS, batch_size, NUM_UNITS]
